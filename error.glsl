...
uniform ARRAY_HIGHP vec2 offset =  vec2 ( 0.0 , 0.0 );
...
ARRAY_HIGHP float noise() {
 return rand ( UV . x + UV . y * 57.0 + TIME * 2.0 ) ;
...
void main()
{
 ...
 highp float aura = exp ( - dist * 4.0 ) ;
 highp float flicker = noise ( uv * 10.0 ) * 0.3 + sin ( time * 5.0 ) * 0.1 ;
 aura += flicker ;
 vec3 color = mix ( vec3 ( 1.0 , 0.3 , 0.0 ) , vec3 ( 1.0 , 1.0 , 0.0 ) , aura ) ;
 color = mix ( vec3 ( 1.0 , 1.0 , 1.0 ) , color , smoothstep ( 0.0 , 1.0 , aura ) ) ;
 COLOR = vec4 ( color , aura ) ;
