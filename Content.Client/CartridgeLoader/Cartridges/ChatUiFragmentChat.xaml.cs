...
public sealed partial class ChatUiFragmentChat : BoxContainer, IChatUiFragmentMode
{
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        if (_toScrollTo is not { } scrollTo)
            return;

        // not initialized
        if (scrollTo.Height <= 0)
            return;

        var y = scrollTo.GlobalPosition.Y - Scroll.Height / scrollTo.Height;

        var toScrollTo = Scroll.GetScrollValue() + new Vector2(0, y);
        Scroll.SetScrollValue(toScrollTo);
