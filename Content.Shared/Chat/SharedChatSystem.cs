...
public abstract class SharedChatSystem : EntitySystem
{
    ...
    /// <summary>
    /// Splits the input message into a prefix part and the rest to preserve it during sanitization.
    /// </summary>
    /// <remarks>
    /// This is primarily for the chat emote sanitizer, which can match against ":b" as an emote, which is a valid radio keycode.
    /// </remarks>
    public void GetKeycodePrefix(EntityUid source,
        string input,
        out string output,
        out string prefix)
    {
        ...
        if (!_radioKeyCodes.TryGetValue(char.ToLower(input[1]), out _) && !_speciesKeyCodes.TryGetValue(char.ToLower(input[1]), out _))
            return;
